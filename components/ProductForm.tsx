...
export default function ProductForm({
  ...
  const [images, setImages] = useState<ItemInterface[]>(existingImages || []);
  ...
  function updateImagesOrder(images: ItemInterface[]) {
    setImages(images);
  }
        ...
        <ReactSortable
          ...
          {!!images?.length && images.map(link => (
            <div key={link?.id} className="h-24 bg-white p-4 shadow-sm rounded-sm border border-gray-200">
              <img src={JSON.stringify(link)} alt="" className="rounded-lg" />
            </div>
          ))}
        </ReactSortable>
        {isUploading && (
